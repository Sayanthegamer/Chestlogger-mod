...
/**
 * Manages active query sessions, mapping transaction logs into bounded, paginated payload frames.
 */
public final class QuerySessionManager {
    ...
    public synchronized PagedResult<DisplayRecord> createSession(
            UUID queryId,
    ...
    public synchronized PagedResult<DisplayRecord> getPage(UUID queryId, int pageIndex) {
        return getPage(queryId, pageIndex, 0);
    }

    public synchronized PagedResult<DisplayRecord> getPage(UUID queryId, int pageIndex, int customPageSize) {
        if (queryId == null) {
            return null;
        }
    ...
    private PagedResult<DisplayRecord> slicePage(SessionEntry session, int requestedPage) {
        int effPageSize = session.pageSize > 0 ? session.pageSize : this.pageSize;
        int total = session.records.size();
        int pages = Math.max(1, (int) Math.ceil((double) total / effPageSize));
        int clampedPage = Math.max(1, Math.min(requestedPage, pages));
        return PagedResult.of(session.records, clampedPage, effPageSize);
    }
