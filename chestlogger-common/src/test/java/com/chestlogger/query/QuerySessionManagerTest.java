...
class QuerySessionManagerTest {
    ...
    void testQuerySessionPagination() {
        ...
        PagedResult<DisplayRecord> page1 = sessionManager.createSession(queryId, "Chest", dim, pos, records, 1);

        assertThat(page1.pageNumber()).isEqualTo(1);
        assertThat(page1.totalPages()).isEqualTo(3); // 60 records / 25 per page = 3 pages
        assertThat(page1.totalElements()).isEqualTo(60);
        assertThat(page1.items()).hasSize(25);

        // Fetch page 2
        PagedResult<DisplayRecord> page2 = sessionManager.getPage(queryId, 2);
        assertThat(page2).isNotNull();
        assertThat(page2.pageNumber()).isEqualTo(2);
        assertThat(page2.items()).hasSize(25);
        assertThat(page2.items().get(0).sequenceId()).isEqualTo(26L);

        // Fetch page 3 (last page with 10 records)
        PagedResult<DisplayRecord> page3 = sessionManager.getPage(queryId, 3);
        assertThat(page3).isNotNull();
        assertThat(page3.pageNumber()).isEqualTo(3);
        assertThat(page3.items()).hasSize(10);
        assertThat(page3.items().get(0).sequenceId()).isEqualTo(51L);

        // Fetch invalid page out of bounds -> clamped to bounds
        PagedResult<DisplayRecord> clamped = sessionManager.getPage(queryId, 99);
