...
7. **Commands & Permissions**: Permission-aware command hierarchy separating regular player container protection commands (`/chestlog claim [radius]`, `/chestlog unclaim`, `/chestlog transfer`, `/chestlog trust`, `/chestlog untrust`, `/chestlog trustlist`, `/chestlog trace [hand]`, `/chestlog help`) from administrative inspection, telemetry, staged rollback, purge, and configuration management tools.
...
11. **Smart Theft & Raid Detection Engine**: Context-aware security evaluation engine featuring ground-truth container placement ownership, directional player trust graphs (`/chestlog trust`, `/chestlog untrust`, `/chestlog trustlist`), sliding 300-second multi-container velocity tracking for `CRITICAL_RAID` bursts, owner distance-based proximity heuristics (`OFFLINE_THEFT`, `ABSENT_OWNER_THEFT`, `CONSENSUAL_PROXIMITY`), multi-channel alerting (in-game Action-Bar HUD, interactive chat with teleport/inspect shortcuts, Discord webhook embeds with presence tags), and Web UI live incident feed (`/api/v1/incidents`) with staged rollback confirmation modal.
12. **Mod Menu & Brand Identity**: First-class Fabric Mod Menu integration embedding custom brand icon assets (`icon.png`), author attribution (`Sayanthegamer`), issue tracker/repository contact endpoints, and `ModMenuApi` configuration hooks linking Mod Menu's in-game "Configure" button to ChestLogger's client GUI (`ChestLogConfigScreen`).
